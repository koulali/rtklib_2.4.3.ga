                   ...
                   const filopt_t *fopt, nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
    ...
    if (*fopt->satantp&&!(readpcv(fopt->satantp,pcvs,popt->navsys))) {
    ...
    if (*fopt->rcvantp&&!(readpcv(fopt->rcvantp,pcvr,popt->navsys))) {
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    /* A.K.12-02-2018: read gpt2 2.5 deg grid ,gpt2_t *gpt2s */
    if (*fopt->gpt2f&&!(readgpt2(fopt->gpt2f,&popt_))) {
        showmsg("error : no values in %s",fopt->gpt2f);
        trace(1,"gpt2 grid read error: %s\n",fopt->gpt2f);
        return 0;
    }
