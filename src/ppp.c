...
static double trop_model_prec(gtime_t time, const double *pos,
                              ...
                              double *var,const prcopt_t *opt)
{
    const double zazel[]={0.0,PI/2.0};
    double zhd,m_h,m_w,cotz,grad_n,grad_e;
    
    /* zenith hydrostatic delay */
    zhd=tropmodel(time,pos,zazel,0.0,opt);
                      ...
                      const nav_t *nav, double *dtrp, double *var)
{
    ...
    if (opt->tropopt==TROPOPT_SAAS) {
        *dtrp=tropmodel(time,pos,azel,REL_HUMI,opt);
        *var=SQR(ERR_SAAS);
        return 1;
    }
    ...
    if (opt->tropopt==TROPOPT_EST||opt->tropopt==TROPOPT_ESTG) {
        ...
        *dtrp=trop_model_prec(time,pos,azel,trp,dtdx,var,opt);
        return 1;
    }
