...
typedef struct {        /*AK-07-01-2018 gpt2 grid structure*/
	double pgrid[2592][5];
	double Tgrid[2592][5];
	double Qgrid[2592][5];
	double dTgrid[2592][5];
	double u[2592];
	double Hs[2592];
	double ahgrid[2592][5];
	double awgrid[2592][5];
} gpt2_t;
...
typedef struct {        /* processing options type */
    ...
    int metopt;         /* pres/temp option for hydrostatic delay*/
...
typedef struct {        /* file options type */
    ...
    char gpt2f  [MAXSTRPATH]; /* gpt2 grid file */
...
EXPORT int  readgpt2(const char *file,prcopt_t *opt);
EXPORT void gpt2(gtime_t time, const double *pos, int it, double *pres, double *temp,gpt2_t *gpt2s);
...
EXPORT int tropcorr(gtime_t time, const nav_t *nav, const double *pos,
                    const double *azel, int tropopt, double *trp, double *var,const prcopt_t *opt);

/* antenna models ------------------------------------------------------------*/
EXPORT int  readpcv(const char *file, pcvs_t *pcvs, int navsys);
