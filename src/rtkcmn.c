...
const prcopt_t prcopt_default={ /* defaults processing options */
    ...
    0,0,0,0,0,                    /* estion,esttrop,dynamics,tidecorr */
...
/* A.Koulali 29-01-2018 ---------------------
*   similar function 'sign' in fortran 
*   from gpt_c.cpp 
*-------------------------------------------*/
extern double sign(double x, double y)
{ 
...
/* read antex file ----------------------------------------------------------*/
static int readantex(const char *file, pcvs_t *pcvs, int navsys)
{
    FILE *fp;
    static const pcv_t pcv0={0};
    pcv_t pcv;
...
*-----------------------------------------------------------------------------*/
extern int readpcv(const char *file, pcvs_t *pcvs,int navsys)
{
    pcv_t *pcv;
    char *ext;
    int i,stat;
    ...
    if (!strcmp(ext,".atx")||!strcmp(ext,".ATX")) {
        stat=readantex(file,pcvs,navsys);
    }
    else {
...
/* read gpt2 grid -----------------------------------------------------------
* A.K 10-02-2018 GPT2
* read gpt2 grid file 2.5 taken from http://ggosatm.hg.tuwien.ac.at/DELAY/SOURCE/
*
* args   : file     I char filename
* opt    : processing options structure (used to save opt->gpt2s)
*-----------------------------------------------------------------------------*/
extern int readgpt2(const char *file,prcopt_t *opt)
{
    ...
    /* read grid file 2.5deg */
    if ( ( grdFile = fopen(file, "r" )) == NULL ) {
        printf( "GPT2 grid could not be opened\n" );
    }

    /* force to pointer to begin of the file */
    rewind(grdFile);  

    /* read the first two lines (grd file in gg format)*/
    for(i=0;i<2;i++) fgets(line,350,grdFile);

    for(n=0;n<2592;n++)
    {
        for (i=0;i<34;i++) fscanf(grdFile, "%lf", &vec[i]);

        for (i = 0; i < 5; i++) opt->gpt2s.pgrid[n][i] = vec[i+2];
        for (i = 0; i < 5; i++) opt->gpt2s.Tgrid[n][i] = vec[i+7];
        for (i = 0; i < 5; i++) opt->gpt2s.Qgrid[n][i] = vec[i+12]/1000.0;
        for (i = 0; i < 5; i++) opt->gpt2s.dTgrid[n][i] = vec[i+17]/1000.0;
        opt->gpt2s.u[n] = vec[22];                                           
        opt->gpt2s.Hs[n] = vec[23];
        for (i = 0; i < 5; i++) opt->gpt2s.ahgrid[n][i] = vec[i+24]/1000.0;  
        for (i = 0; i < 5; i++) opt->gpt2s.awgrid[n][i] = vec[i+29]/1000.0; 
    }
    fclose(grdFile);
    return 1;
}
...
* 
* AK-29-01-2018: - Use the 2.5 resolution
*                - Use the same grid file format as in ~/gg/gamit/model/
*                - Use one variable instead of arrays for stations
*
* TODO:         - find a way how to speedup reading the grid for each time
*                epoch; It is too slow for the moment.
*
*     args:     time:       I rtklib time
*               pos :       I position array{lat,lon,dhgt( in m)}
*
*     return:   pres:       I pressure in hPa
*               temp:       I temperature in Celsius
*               dT:         I temperature lapse rate in degrees per km (vector of length nstat)
*               e:          I water vapour pressure in hPa (vector of length nstat)
*               ah:         I hydrostatic mapping function coefficient at zero height (VMF1)
*                             (vector of length nstat)
*               aw:         I wet mapping function coefficient (VMF1) (vector of length nstat)
*               undu:       I geoid undulation in m (vector of length nstat)
*
*     Reference:
*     Lagler, K., Schindelegger, M., Böhm, J., Krásná, H., Nilsson, T. (2013), 
*     GPT2: Empirical slant delay model for radio space geodetic techniques, 
*     Geophys. Res. Lett., Vol. 40, pp. 1069–1073, DOI: 10.1002/grl.50288.
*
*    translated from : http://ggosatm.hg.tuwien.ac.at/DELAY/SOURCE/gpt2.f
*-----------------------------------------------------------------------------*/
extern void gpt2(gtime_t time, const double *pos, int it, double *pres, double *temp,gpt2_t *gpt2s)
{   
    
    const double ep[]={2000,1,1,12,0,0};
    double gm,dMtr,Rg,dlat,dlon,hell,dmjd1;
    double ppod=0.0,plon=0.0,diffpod=0.0,difflon=0.0;
    double cosfy=0.0,coshy=0.0,sinfy=0.0,sinhy=0.0;
    double ah=0.0,aw=0.0,undu=0.0;
    double hgt=0.0, T0=0.0, p0=0.0,Q=0.0, redh=0.0, Tv=0.0, c=0.0;
    double dT=0,e=0.0,Hs1=0.0,dnpod1=0.0,dnpod2=0.0,R1=0.0,R2=0.0,
    dnlon1=0.0,dnlon2=0.0;
    double undul[4],Ql[4],dTl[4],Tl[4],pl[4],ahl[4],awl[4];
    

    int l;
    int ipod,ipod1,ilon,ilon1;
    int indx[4];
    int ibilinear=0, ix=0;


    gm = 9.806650; /* mean gravity in m/s**2 */
    dMtr = 28.965e-3; /* molar mass of dry air in kg/mol */
    Rg = 8.31430; /* universal gas constant in J/K/mol */

    dlat = pos[0];
    dlon = pos[1];
    hell = pos[2]<0.0?0.0:pos[2];

    /* change the reference epoch to January 1 2000 */
    dmjd1=(timediff(time,epoch2time(ep)))/86400.0;
    ...
    /*find the index (line in the grid file) of the nearest point*/
    ipod = floor((ppod+5.0)/5.0);
    ilon = floor((plon+5.0)/5.0);
    ...
    ibilinear = 0;
    if( (ppod > 2.5) && (ppod < 177.5) ) ibilinear = 1;
    ...
    if (ibilinear == 0) 
    {
        ...
        T0 = gpt2s->Tgrid[ix][0] +
           gpt2s->Tgrid[ix][1]*cosfy +gpt2s->Tgrid[ix][2]*sinfy +
           gpt2s->Tgrid[ix][3]*coshy +gpt2s->Tgrid[ix][4]*sinhy;

        p0 = gpt2s->pgrid[ix][0] +
           gpt2s->pgrid[ix][1]*cosfy + gpt2s->pgrid[ix][2]*sinfy+
           gpt2s->pgrid[ix][3]*coshy + gpt2s->pgrid[ix][4]*sinhy;    

        /* specific humidity */
        Q = gpt2s->Qgrid[ix][0] +
            gpt2s->Qgrid[ix][1]*cosfy + gpt2s->Qgrid[ix][2]*sinfy+
            gpt2s->Qgrid[ix][3]*coshy + gpt2s->Qgrid[ix][4]*sinhy;

        /* lapse rate of the temperature */
        dT = gpt2s->dTgrid[ix][0] +
            gpt2s->dTgrid[ix][1]*cosfy + gpt2s->dTgrid[ix][2]*sinfy+
            gpt2s->dTgrid[ix][3]*coshy + gpt2s->dTgrid[ix][4]*sinhy;

        /* station height - grid height */
        redh = hgt - gpt2s->Hs[ix];
        

        /* temperature at station height in Celsius */
        *temp = T0 + dT*redh - 273.150;
        ...
        /* pressure in hPa */
        *pres = (p0*exp(-c*redh))/100.0;
        
        /* water vapour pressure in hPa */
        e = (Q*(*pres))/(0.6220 + 0.3780*Q);

        /* hydrostatic coefficient ah */
        ah = gpt2s->ahgrid[ix][0] +
                gpt2s->ahgrid[ix][1]*cosfy + gpt2s->ahgrid[ix][2]*sinfy+
                gpt2s->ahgrid[ix][3]*coshy + gpt2s->ahgrid[ix][4]*sinhy;

        /* wet coefficient aw */
        aw = gpt2s->awgrid[ix][0] +
                gpt2s->awgrid[ix][1]*cosfy + gpt2s->awgrid[ix][2]*sinfy +
                gpt2s->awgrid[ix][3]*coshy + gpt2s->awgrid[ix][4]*sinhy;
    ...
    else
    {
        ...
        do{
            ...
            dTl[l] = gpt2s->dTgrid[ix][0] +
                ...
                /* hydrostatic coefficient ah */
                ahl[l] = gpt2s->ahgrid[ix][0] +
                         gpt2s->ahgrid[ix][1]*cosfy + gpt2s->ahgrid[ix][2]*sinfy +
                         gpt2s->ahgrid[ix][3]*coshy + gpt2s->ahgrid[ix][4]*sinhy;

                /*wet coefficient aw*/
                awl[l] = gpt2s->awgrid[ix][0] +
                         gpt2s->awgrid[ix][1]*cosfy + gpt2s->awgrid[ix][2]*sinfy +
                         gpt2s->awgrid[ix][3]*coshy + gpt2s->awgrid[ix][4]*sinhy;
...
extern double tropmodel(gtime_t time, const double *pos, const double *azel,
                        double humi,const prcopt_t *opt)
{
    const double temp0=15.0; /* temparature at sea level */
    double hgt,pres,temp,e,z,trph,trpw;

    double mjd,lat,lon,hgtm,und;
    int it=0;
    ...
    else if (opt->metopt==1)
    { /*A.K 10-02-2018 GPT*/
        mjd=51544.5+(timediff(time,epoch2time(ep)))/86400.0;
        lat=pos[0];
        lon=pos[1];
        hgtm=pos[2]-geoidh(pos); /* height in m (mean sea level) */
        gpt_(&mjd,&lat,&lon,&hgtm,&pres,&temp,&und);
    }
    else if (opt->metopt==2)
    { /*A.K 13-02-2018 GPT2*/
        gpt2s = opt->gpt2s;
        gpt2(time,pos,it,&pres,&temp,&gpt2s);
    }
